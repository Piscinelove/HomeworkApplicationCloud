...
public class TeacherEndpoint {
     ...
     *
     * @param teacherId the ID of the entity to be retrieved
    ...
    public Teacher get(@Named("teacherId") long teacherId) throws NotFoundException {
        logger.info("Getting Teacher with ID: " + teacherId);
        Teacher teacher = ofy().load().type(Teacher.class).id(teacherId).now();
        if (teacher == null) {
    ...
    public Teacher insert(Teacher teacher) {
        ...
        // You should validate that teacher.teacherId has not been set. If the ID type is not supported by the
        // Objectify ID generator, e.g. long or String, then you should generate the unique ID yourself prior to saving.
        //
        // If your client provides the ID then you should probably use PUT instead.
        ofy().save().entity(teacher).now();
        logger.info("Created Teacher with ID: " + teacher.getTeacherId());
     ...
     *
     * @param teacherId the ID of the entity to be updated
     * @param teacher   the desired state of the entity
     * @return the updated version of the entity
     * @throws NotFoundException if the {@code teacherId} does not correspond to an existing
     *                           {@code Teacher}
     */
    @ApiMethod(
            name = "update",
    ...
    public Teacher update(@Named("teacherId") long teacherId, Teacher teacher) throws NotFoundException {
        // TODO: You should validate your ID parameter against your resource's ID here.
        checkExists(teacherId);
        ofy().save().entity(teacher).now();
     ...
     *
     * @param teacherId the ID of the entity to delete
     * @throws NotFoundException if the {@code teacherId} does not correspond to an existing
     *                           {@code Teacher}
     */
    @ApiMethod(
    ...
    public void remove(@Named("teacherId") long teacherId) throws NotFoundException {
        checkExists(teacherId);
        ofy().delete().type(Teacher.class).id(teacherId).now();
    ...
    private void checkExists(long teacherId) throws NotFoundException {
        try {
            ofy().load().type(Teacher.class).id(teacherId).safe();
